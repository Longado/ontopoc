...
Pain：人工判断与验收结果不一致，指标确认与验收有断点，领域对应草稿和部分 RDF 声明会丢失。

Decision：当前本体与人工对应是否足以支撑用户认可的业务问题？

Outcome：验收遵守人工判断和已确认指标；MCP 能完整传递待确认公式；切换页面保留领域对应草稿；不支持的 RDF 约束明确提示。

Non-goals：不增加新功能、不做 OWL 推理、不改模型提示或增加模型调用。

Smallest path：修正现有验收、问答落盘、MCP 返回字段、RDF 解析与检查页面的组件生命周期。

- 保存、重跑验收时使用人工判断后的本体及已确认指标；改变判断后重新核验已存验收。历史问答保留当时结果，将其存为验收时按最新判断重新执行。
- 问答等待期间人工判断发生变化，返回 409 并提示重新提问，不保存旧口径答案。
- MCP 的提问和历史问题返回完整 `derive`，包括公式与试算，人审阅后可原样确认。
- 领域对应草稿在同一运行内切换检查页签或模块后保留；取消修改恢复已保存记录，切换运行不继承草稿。
- 多个 RDF domain/range 不再只采用第一个；未转换约束明确警告，完整声明保留在原文。

回归测试与实现分开提交。真实模型试跑和离屏截图保存在本机忽略目录 `output/platform-fixes-qa/`，使用独立临时数据目录，不改正式运行数据。
