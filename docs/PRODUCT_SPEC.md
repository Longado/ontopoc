...
- 首个垂直场景：供应链订单优先干预；
- 当前核心产品瞬间：真实材料产生模板外、可追溯且比人工值得的判断与追问。
...
验证参与者：供应链业务负责人负责判断材料输出是否可理解、可纠正、可复用；售前、解决方案架构师、数据负责人、产品经理和实施负责人暂不作为 MVP 独立用户流程。
...
- Loop 0 基线行为持续由回归测试保护，默认 CLI 未指定知识包时保持兼容；
...
- 真实材料产生的 role binding 以材料抽取为主、可追到原文，同一材料重跑保持稳定，知识追问有明确命中且无误触；
- 同一材料的管道 Markdown 覆盖不低于用户手写 5 条判断且耗时不超过 3 倍；
- 一名真实 FDE 和一名供应链业务验证参与者共同形成可记录的 `go`：保存实验输入 hash、纠正或复用证据、理由和时间；否则 MVP 验证失败，不进入 `DecisionDelta`、数据接入、行动和 API。
...
这次 validation 完成没有改变核心产品风险：输出结构仍大部分模板化。当前唯一主线是依次验证 J1 材料贡献、J2 知识命中和 J3 相对人工的价值；任一阶段失败即停止扩张。在真实 FDE 与业务验证参与者形成可记录的判断前，不得把页面、回执或测试快照写成产品价值已经通过。
