...
本产品把 AI FDE 对业务问题的理解编译成 `DecisionPack`，再逐步形成可执行规格、验证回执和业务变化说明。Loop 1 将使 POC Markdown 成为 `DecisionPack` 的兼容投影；当前已实现的 Markdown 仍是 Proposal CLI 输出。
...
当前仓库只完成第一项基线；其余均为 Loop 1–4 的目标，不得写成已实现能力。
