...
| ValidationReceipt schema | LIVE (`synthetic_demo`) | 固定 baseline/candidate pack/spec 与 4 组 synthetic facts | 8 个 canonical `validation_receipt.v1`，绑定 pack/spec/facts hash、rule、fact refs 与 evidence refs | 回执只证明固定合成求值已记录；不得称为人工审查、客户验证、发布或生产回执 |
| Deterministic validation evaluator | LIVE (`recorded synthetic_demo`) | `categorical_all_of_v1` candidate rule 与固定 synthetic facts | `pass / fail / not_evaluable / unsupported` 四态结果 | evaluator 只在 Python artifact 生成时运行；浏览器不执行规则；不得称为实时客户规则运行 |
