...
- 2026-09-13 已从概念板切出两张透明底 PNG 替换站点 Logo：`landing-page/public/assets/ontopoc-logo.png`（深底用，浅色字）和 `ontopoc-logo-light.png`（页脚等浅底用）；原来的反色滤镜已去掉。favicon、单色版、16/32px 小图标仍未做。像素风用于标识及适量标题，业务表格和正文优先可读性。
...
> 请先读 `docs/superpowers/plans/2026-09-13-nhtsa-shortcomings-and-next-rounds.md`（问题清单、已完成的"让电池和刹车答案可信"一轮、下一轮）、`docs/NHTSA_AUTO_ONTOLOGY_PLAN.md`、`docs/DEVELOPMENT_GUARDRAILS.md`，确认当前工作树和分支。下一轮是该文档第 3 节"给一位质量岗用 30 分钟"：字段中文名、下载结果带日期/起火/碰撞/系列、其余系列跑原文判断、用第一批人工复核做固定回归样本。先写失败测试再实现；不混入企业字段或部署。DeepSeek 本机凭据位置见第 9 节，禁止输出密钥。先检查已有改动，不覆盖其他工作树。
