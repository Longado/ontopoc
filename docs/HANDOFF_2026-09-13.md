...
```text
主仓库：/Users/eddie/Desktop/Workspace/ontology-poc-generator
当前开发工作树：/Users/eddie/Desktop/Workspace/ontology-poc-generator/.worktrees/recall-scope-deepseek
...
- 本机文件：`/Users/eddie/.local-sensitive/ontopoc/deepseek.env`，文件 0600、目录 0700，位于仓库外。
...
(
  set -a
  source /Users/eddie/.local-sensitive/ontopoc/deepseek.env
  set +a
...
> 请先读 `docs/HANDOFF_2026-09-13.md` 与 `docs/DEVELOPMENT_GUARDRAILS.md`，确认当前工作树和分支。接下来仅执行接力文档第 6 节的工作台操作可靠性改进：筛选/详情同步、长清单详情可见、复核草稿保护、手机列表与详情返回。先写计划与边界，再补失败测试、实施并做 24 行浏览器验收。保留原匹配 API 和计算逻辑，不混入企业字段、新事件、Logo 替换或部署。Logo 概念稿已获用户认可。DeepSeek 本机凭据位置见第 9 节，禁止输出密钥。先检查已有改动，不覆盖其他工作树。
