...
**每句话都是算出来的，不是说出来的。** 数据体检由代码逐行算六项：字段有没有去处、同一编号信息打不打架、编号写法是否统一、关系是否真连上、引用的对象找不找得到、表和表连不连得上。然后模型把业务问题写成结构化查询，答案由代码在数据上算出（数个数、算占比、求和、求平均、按几样东西分组）；答不了时写明是本体缺了、数据里没有，还是这种问法还不支持。
...
| `src/ontology_poc_generator/ontology_eval.py` | 六项数据体检，逐行算 |
