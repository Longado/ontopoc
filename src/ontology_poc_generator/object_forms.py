...
(docs/PLATFORM_FORM_REFERENCE.md). Key, type and length are read from the data (handover_form); the names,
...
PLATFORM_TYPES = {'INTEGER': 'INT', 'DECIMAL': 'DOUBLE', 'VARCHAR': 'VARCHAR', 'DATE': 'DATE', 'DATETIME': 'DATETIME'}


def form_files(ontology: dict, handover: dict, form: dict | None) -> dict[str, str]:
    ...
    for t in handover.get('types', []):
        ...
        for f in t['fields']:
            ...
            rows.append(['是' if f['identity'] else '', '是' if own.get('display_field') == f['path'] else '', mine.get('label') or '',
                         f['path'], mine.get('description') or '', PLATFORM_TYPES.get(f.get('type'), ''), str(f['length']) if f.get('type') else '',
        ...
        if t.get('needs_single_key'):
            splits.append(f"{key}.csv：靠 {' + '.join(t['identity_fields'])} 一起识别。目标平台每张表只收一个主键时，导入前把它们合成一个字段，或改建模。")
    files['说明.txt'] = '\n'.join([
        'OntoPoc 导出的对象表单，每个对象一张 CSV，列为数据平台建对象时常见的属性表单列。',
        '类型、长度、主键由代码逐行读数据得到；中文名称、描述、展示字段是人写的或人留下的起草稿，空着的要在目标平台里补。',
        '导入前请实测：目标平台批量导入的文件格式、主键规则、关系表达方式都需要先与平台确认；关系表因此没有导出。',
