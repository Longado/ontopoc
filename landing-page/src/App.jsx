...
const content = {
  zh: {
    ...
    heroAlt: "供应链对象关系向订单判断传播的点阵", heroVision: "规则一变，立即找出受影响订单；业务确认后生效。",
    ...
    proofEyebrow: "SUPPLY CHAIN SCENARIO", proofTitle: ["承诺期从 7 天变为 12 天，", "哪张订单要转入优先干预？"], proofLead: "OntoPoc 沿供应商—物料—订单关系传播影响，重算订单 SC-2188 的处置结论，并在新结论生效前交给供应链负责人确认。",
    techPillars: [["RELATION GRAPH", "沿对象关系传播", "供应商 → 物料 → 订单"], ["RECALCULATE", "重算业务判断", "新规则与旧结论同场验证"], ["HUMAN CONTROL", "确认后才生效", "不自动改写 ERP"]],
    modelEyebrow: "WHY IT COMPOUNDS", modelTitle: ["修正留得住，", "复核才会越来越快。"], modelLead: "规则版本、影响路径和业务确认保存在同一份决策记录里。不是知识库越做越大，而是每个供应链判断越用越清楚。",
  ...
  en: {
    ...
    heroAlt: "A halftone field showing impact moving through supply-chain relationships", heroVision: "When a rule changes, find the affected orders—and confirm the outcome before it takes effect.",
    ...
    proofEyebrow: "SUPPLY CHAIN SCENARIO", proofTitle: ["PROMISE WINDOW: 7 TO 12 DAYS.", "WHICH ORDER NOW NEEDS INTERVENTION?"], proofLead: "OntoPoc propagates the change through supplier–material–order relationships, recalculates the judgment for order SC-2188, and routes the candidate outcome to the supply-chain owner before it takes effect.",
    techPillars: [["RELATION GRAPH", "Propagate through objects", "Supplier → Material → Order"], ["RECALCULATE", "Recompute the judgment", "New rule against prior outcome"], ["HUMAN CONTROL", "Take effect after confirmation", "No silent ERP writeback"]],
    modelEyebrow: "WHY IT COMPOUNDS", modelTitle: ["EVERY BUSINESS CORRECTION", "MAKES THE NEXT REVIEW FASTER."], modelLead: "Rule versions, impact paths, and business confirmations stay in one decision record. The knowledge base does not simply grow; each supply-chain judgment becomes easier to recheck.",
  ...
  zh: {
    model: {
      ...
      stages: [
        ...
        { label: "CAPTURE", status: "保留修正依据", headline: "把 7 天到 12 天的修正留下来", body: "规则版本、业务证据、影响路径和确认人进入同一份可回溯记录。", metric: "v0.9.1", receipt: "CORRECTION / RECEIPT 17" },
        { label: "REUSE", status: "再次发生变化", headline: "下次不用重新从头排查", body: "系统沿既有关系重算影响，只把需要确认的订单和原因交给负责人。", metric: "5 个对象", receipt: "REVIEW / QUEUE 03" },
      ],
      objectSet: "业务对象",
      relation: "对象关系",
      evidence: "证据与 Action",
  ...
  en: {
    model: {
      ...
      stages: [
        ...
        { label: "CAPTURE", status: "Keep the correction evidence", headline: "Preserve the correction from 7 to 12 days", body: "The rule version, business evidence, impact path, and confirming owner stay in one recoverable record.", metric: "v0.9.1", receipt: "CORRECTION / RECEIPT 17" },
        { label: "REUSE", status: "The next change arrives", headline: "Do not restart the investigation from scratch", body: "OntoPoc follows existing relationships and returns only the affected orders and reasons for confirmation.", metric: "5 objects", receipt: "REVIEW / QUEUE 03" },
      ],
