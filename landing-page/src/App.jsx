...
const copy = {
  zh: {
    ...
    problems: [
      ...
      ["初判不能当结论", "模型读原文会摆动，同一提示词跑两次约一成结果不同，必须有人复核。"],
    ],
    methodKicker: "做法",
    ...
    facts: [["17", "类本体错误检查。两份数据上模型都是第 1 次提交被退回、第 2 次通过"], ["0 → 34", "Bolt 刹车召回的候选投诉，靠一条核验过的名称对应补回；同一条对应在 Kona 上补回 11 条"], ["43", "条 Bolt 电池投诉落在所有召回之外、都在召回之后。这是待复核的候选，模型初判 41 条不是同一缺陷、2 条说不清"], ["10", "条 Kona 2021 款电池投诉落在两个电池召回之外，因为这两个召回只覆盖 2019–2020 款"]],
    limitsKicker: "边界",
    limits: [
    ...
    source: "数据来自 NHTSA 公开接口，2026-09-13 取数。OntoPoc 与 NHTSA、通用汽车、现代汽车无关联，页面内容不是官方结论，也不构成缺陷认定。",
  },
  en: {
    ...
    problems: [
      ...
      ["First calls drift", "The same prompt run twice changes about one verdict in ten; a human has to review."],
    ],
    methodKicker: "METHOD",
    ...
    facts: [["17", "kinds of ontology error checked; on both datasets the model was sent back once and passed on the second attempt"], ["0 → 34", "Bolt brake recall candidates recovered by one verified alias; the same alias recovers 11 on Kona"], ["43", "Bolt battery complaints outside every recall, all filed after it. They are candidates to review: the model's first call is 41 not the same defect, 2 unclear"], ["10", "Kona 2021 battery complaints outside both battery recalls, which cover only 2019–2020"]],
    limitsKicker: "LIMITS",
    limits: [
    ...
    footer: "Trace scope. Review evidence.",
    source: "Data from the NHTSA public API, fetched 2026-09-13. OntoPoc is not affiliated with NHTSA, General Motors or Hyundai; nothing here is an official finding or a defect determination.",
  },
};
