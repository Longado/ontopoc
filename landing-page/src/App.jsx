...
const copy = {
  zh: {
    ...
    lead: "给实施顾问和本体团队用：交一份客户的数据表（CSV、Excel）或业务文档（Markdown、Word、PDF）。模型提出本体里有哪些对象、按什么识别、彼此怎么关联，代码拿每一行数据核验；然后自动做数据体检和业务问答，有人写的标准答案时再逐项对照。",
    problemKicker: "问题",
    ...
    problems: [
      ...
      ["每次跑出来不一样", "同一份文件重跑，模型搭的本体会有出入。示例公司同时建了三次，其中一次多搭了一个“售后工程师”对象。"],
    ],
    methodKicker: "做法",
    ...
    facts: [["4 · 3", "个对象、条关系。模型第 1 版有字段没交代去处、关系写错了表，被代码退回，第 2 版通过"], ["2", "处埋下的问题都被数据体检查出：客户 KH012 在两行里城市不同，订单引用的 KH099 不在客户表里"], ["6 / 6", "道模型出的业务问题由代码在数据上答出，包括同时按客户和产品分组的一道，和一道算已关闭工单占比的"], ["4 / 5", "个对象和人写的参考本体对上；少了“售后工程师”，因为数据里工程师只是一列姓名"]],
    exampleKicker: "示例场景",
    ...
    limits: [
      ...
      "模型每次搭的本体会有出入；每次上传同时建三次，标出不是每次都有的对象，并列出和上一次的差别。",
      "表格和文档现在分开建模，还不能把同一业务的表和文档合在一起。",
      "除了合成的示例公司，还在公开的真实数据上试过（芝加哥市政府合同 3000 条、一家上市公司的关联交易管理制度、英国零售交易、Chinook 发票 412 条），每份都逼出过问题并已修掉：按部门求和的 46.3 亿与 Python 逐行相加一分不差，名称对应在芝加哥合同上端到端跑通并在重传时带回了上次的确认。还没有真实客户用过。",
      "上传的文件只存在本机。发给配置的模型接口的，只有每列的字段名和每列最多 3 个示例值、取值不超过 12 种的列的全部取值（文档则是正文）；上传前可以在上传页展开'看看会发给模型什么'逐列查看，真实客户数据先按这个判断要不要脱敏。",
    ],
    cta: "打开工作台，上传文件或先看示例公司",
    footer: "Draft the ontology. Show the evidence.",
  ...
  en: {
    ...
    problems: [
      ...
      ["Every run differs", "Rerun on the same file and the model's ontology changes. The demo company was modelled three times at once, and one run added an “after-sales engineer” object."],
    ],
    methodKicker: "METHOD",
    ...
    facts: [["4 · 3", "objects and relations. Code sent the first proposal back for unaccounted fields and a relation on the wrong table; the second passed"], ["2", "planted problems both caught by the data check: customer KH012 has two cities, and orders reference KH099, which is not in the customer table"], ["6 / 6", "model-written questions answered by code on the data, including one grouped by customer and product at once and one share of closed tickets"], ["4 / 5", "objects match the hand-written reference; the missing one is “after-sales engineer”, which the data holds only as a name column"]],
    ...
    limits: [
      ...
      "The model's ontology varies between runs; each upload is modelled three times, parts missing from some runs are marked, and differences from the previous run are listed.",
      "Tables and documents are modelled separately; one business's tables and documents cannot be combined yet.",
      "Beyond the synthetic demo company, tried on real public data (3,000 Chicago city contracts, a listed company's related-party transaction policy, UK retail transactions, 412 Chinook invoices), each of which exposed problems that are now fixed: a per-department total of 4.63 billion matches adding the rows up in Python, and name matching ran end to end on the Chicago contracts and carried last time's confirmation back on a re-upload. No real client has used it yet.",
      "Uploaded files stay on this machine. Only each column's name, at most 3 example values per column and the full value list of columns with at most 12 values (for documents, the text) are sent to the configured model API; the upload page can list exactly this before sending, so decide from it whether real client data needs masking.",
    ],
