...
const copy = {
  zh: {
    ...
    facts: [["17", "类本体错误检查。Bolt 上模型第 1 次提交被退回、第 2 次通过；Kona 这次第 1 次就通过"], ["0 → 34", "Bolt 刹车召回的候选投诉，靠一条核验过的名称对应补回；同一条对应在 Kona 上补回 12 条"], ["43", "条 Bolt 电池投诉落在所有召回之外、都在召回之后。这是待复核的候选，模型初判 41 条不是同一故障、2 条说不清"], ["10", "条 Kona 2021 款电池投诉落在两个电池召回之外，因为这两个召回只覆盖 2019–2020 款"]],
    limitsKicker: "边界",
    limits: [
  ...
  en: {
    ...
    problems: [
      ...
      ["Recalls chain", "Five recalls for one problem reference each other; which repair failed is only in the prose."],
    ...
    steps: [
      ["Auto-built ontology", "The model sees only field names and samples and proposes objects, identities, relations, time fields and filters; code checks all records for 17 kinds of error and sends errors back, three attempts at most."],
      ...
      ["First call and review", "A “same failure” verdict must quote the complaint, and code checks the quote is really there; the engineer reviews each one, and those reviews become the calibration labels."],
    ],
